...
import json
import time  # Added for the pause at exit
...
if not video_files:
    ...
    console.print(f"[bold yellow]Please add video files to the directory and run the script again.[/bold yellow]")
    # Pause before exit to let user read the message, especially in PowerShell
    console.print("\n[bold white]Press any key to exit...[/bold white]")
    # Use msvcrt for Windows systems to capture any keypress without requiring Enter
    try:
        import msvcrt
        msvcrt.getch()
    except ImportError:
        # Fall back to input() for non-Windows systems
        input()
    exit(1)
...
# Pause at the end to prevent immediate closure in PowerShell
console.print("\n[bold white]Press any key to exit...[/bold white]")
# Use msvcrt for Windows systems to capture any keypress without requiring Enter
try:
    import msvcrt
